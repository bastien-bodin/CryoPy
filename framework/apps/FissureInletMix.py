...
from modules.Geometries import MyFissure2D, MyBlock2D, MyPit2D
from modules.FluidDynamics import (
    ...
    InletVelocity, ViscositySnowMix, ViscosityBinghamRho, VelGrad2D
)
...
class FissureInlet(Application):
    def __init__(self, fname=None, output_dir=None, domain=None,
                 ...
                 tf:float=10.0, t_inlet:float=5.0):
        ...
        super(FissureInlet, self).__init__(fname, output_dir, domain)
        
    def initialize(self):
        self.hdp = 1.5
        self.h = self.dp*self.hdp
        self.gx, self.gy = 0.0, -1.315
        self.eps = 0.8
    ...
    def create_particles(self):
        Xb, Yb = MyPit2D(
            ...
            lx_pit=self.lx, ly_pit=self.ly,
            dp =self.dp, nl=2
        )
        x0pit = self.x0 + 0.5*(self.Lx-self.lx)
        y0pit = self.y0 - self.ly
        Xf, Yf = MyBlock2D(
            x0=x0pit, y0=y0pit,
        ...
        inlet = get_particle_array_wcsph(
            name='inlet', x=Xf, y=Yf, h=self.h, m=self.mass,
            rho=self.rho0, cs=self.c0
        )
        
        PA = [fluid, boundaries, inlet]
    ...
    def create_equations(self):
        ...
        equations = [
            ...
            Group(equations=[
                VelGrad2D(dest='fluid', sources=['fluid', 'bound', 'inlet']),
                VelGrad2D(dest='bound', sources=['fluid', 'inlet']),
                VelGrad2D(dest='inlet', sources=['fluid', 'bound']),
            ]),
            Group(equations=[
                ViscositySnowMix(dest='fluid', sources=None, rho0=self.rho0,
                                 xc=self.Lx/2.0, lx=self.lx),
                ViscosityBinghamRho(dest='inlet', sources=None, rho0=self.rho0),
            ]),
            Group(equations=[
                ViscosityMean(dest='bound', sources=['fluid', 'inlet']),
            ]),
            Group(equations=[
                ...
                TaitEoS(dest='bound',sources=None, rho0=self.rho0,
                        c0=self.c0),
                TaitEoS(dest='inlet',sources=None, rho0=self.rho0,
                        c0=self.c0),
            ]),
            Group(equations=[
                ...
                ContinuityEquation(
                    dest='bound',sources=['fluid', 'inlet'],
                ),
                ContinuityEquation(
                    dest='inlet',sources=['fluid', 'bound'],
                ),
                MomentumMorris(
                    dest='fluid', sources=['fluid', 'bound', 'inlet'],
            ...
            Group(equations=[
                AdaptiveTSMorris(dest='inlet', sources=None),
